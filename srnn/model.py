...
class SRNN(nn.Module):
    ...
    def __init__(self, args,  infer=False):
        ...
        if self.infer:
            ...
            # self.pred_length = args.seq_length - args.obs_length
    ...
    def forward(
        ...
        outputs = Variable(torch.zeros(current_seq_length * numNodes, self.output_size))
        if self.use_cuda:
            outputs = outputs.cuda()

        # Data structure to store attention weights
        attn_weights = [{} for _ in range(current_seq_length)]
        ...
        outputs_return = Variable(
            torch.zeros(current_seq_length, numNodes, self.output_size)
        )
        if self.use_cuda:
